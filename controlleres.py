...
from  modelos  import  insert_estado , get_estado , insert_usuario , get_usuario , update_usuario , delete_usuario , select_usuarios
from  serializadores  import  estado_from_web , estado_from_db , cidades_from_web,cidades_from_db,usuario_from_web , usuario_from_db , nome_usuario_from_web,
...
def inserir_filmes ("filmes", ["id", "titulo","ano ","classificação","preço","diretores","diretores_id","generos_id"],
...
def  inserir_pagamento ("pagamento", [ "id","status","cod pagamento", "valor","data","locação_id "] )
            pagamento_from_web(**request.json " pagamento")
            tipo = tipo_from_web( ** request.json  "tipo" )
            status= status_from_web( ** request.json "status" )
            cod_pagamento = cod_pagamento_from_web ( ** request.json "cod_pagamento" )
            pagamento = pagamento_from_web( ** request.json  "pagamento" )
            valor = valor_from_web( ** request.json "valor")
            data = data_from_web( ** request.json "data" )
            locacoes_id = locacoes_id_from_web ( ** request.json "data")
...
@ app . rota ( "/ cidades / <varchar: sigla,nome>" , métodos = [ "PUT" , "PATCH" ])
def  alterar_cidades ( sigla,nome ):
    cidade  =  cidades_from_web ( ** request . json )
    if  valida_cidade ( ** cidade ):
        update_cidade ( sigla ,nome  ** sigla,nome )
        cidade_cadastrado  =  get_cidades ( sigla,nome )
        return  jsonify ( cidade_from_db ( estado_cadastrado ))
    else :
        return  jsonify ({ "erro" : "cidade inválido" })
...
@ app . rota ( "/ cidades " , métodos = [ "GET" ])
def  buscar_cidade ():
    sigla  =  sigla_from_web ( ** solicitação . args )
    estado  =  select_cidade ( sigla,nome )
    estados_from_db  = [ estado_from_db ( usuario ) para  estado  em  estados ]
    return  jsonify ( estado_from_db )
...
@app.route("/cidades/<varchar: sigla , nome>", methods=["PUT"])
def alterar_cidade(sigla,nome):
    pass

@app.route("/cidades/<varchar: sigla, nome >", methods=["DELETE"])
def apagar_cidade(id):
    pass
